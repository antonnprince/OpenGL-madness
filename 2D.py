...
def plot_points(points=([0.3, 0.3, -0.3, -0.3], [-0.3, 0.3, 0.3, -0.3])):
    ...
    glFlush()
    option = int(
        input("enter 1-translation 2-rotation 3-reflection 4-scaling 5-shearing")
    )
    if option == 1:
        ...
        for i in range(len(points[0])):
            points[0][i] = points[0][i] * cos(rad(angle)) + points[1][i] * sin(
                rad(angle)
            )
            points[1][i] = points[1][i] * cos(rad(angle)) - points[0][i] * sin(
                rad(angle)
            )
    elif option == 3:
...
def main():
    ...
    glutDisplayFunc(lambda: plot_points())
    glutMainLoop()
